...
export default function UploadTool() {
  ...
  // Fetch current usage on page load -- without this, the banner only
  // populated after an actual upload completed, so a fresh page load
  // (e.g. right after upgrading) showed nothing instead of "X left."
  useEffect(() => {
    fetch("/api/usage")
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data) {
          setUploadsUsed(data.uploadsUsed);
          setUploadsLimit(data.uploadsLimit);
        }
      })
      .catch(() => {
        // Silently ignore -- the banner just won't show until the first
        // upload completes, which is the old (acceptable) behavior.
      });
  }, []);
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: C.sand, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <nav style={{ borderBottom: `2px solid ${C.black}`, background: C.sand }}>
          <div style={{ maxWidth: "780px", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.5rem" }}>
            <Link href="/" style={{ fontFamily: "'Fraunces', serif", fontSize: "1.2rem", fontWeight: 900, color: C.black, textDecoration: "none" }}>
              TalkToHook
            </Link>
            <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
              <Link href="/history" style={{ fontSize: "0.82rem", fontWeight: 600, color: C.black, textDecoration: "none" }}>
                History
              </Link>
              <AuthButton />
            </div>
        ...
        <main style={{ maxWidth: "780px", margin: "0 auto", padding: "3rem 1.5rem" }}>
          <header style={{ marginBottom: "2rem" }}>
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "2.2rem", fontWeight: 900, letterSpacing: "-0.02em", color: C.black }}>
              Find your hook
            </h1>
            <p style={{ marginTop: "0.5rem", fontSize: "0.95rem", color: "#555", lineHeight: 1.6 }}>
              Upload a talking video. We&rsquo;ll transcribe it and rank the best lines as ready-to-post hooks, with a caption for each.
            </p>
          ...
          {state === "limit_reached" ? (
            <section style={{ border: `2px solid ${C.black}`, background: C.cherry, padding: "2.5rem", textAlign: "center" }}>
              <p style={{ fontFamily: "'Fraunces', serif", fontSize: "1.5rem", fontWeight: 900, color: C.white }}>
                You&rsquo;ve used all {uploadsLimit} uploads
              </p>
              <p style={{ marginTop: "0.75rem", fontSize: "0.9rem", color: C.pink }}>
                Upgrade to keep finding hooks in your videos.
              </p>
              <div style={{ marginTop: "1.5rem", maxWidth: "240px", marginLeft: "auto", marginRight: "auto" }}>
                <UpgradeButton plan="pro" style={{ background: C.white, color: C.cherry, padding: "0.85rem 2rem" }}>
                  Upgrade to Pro
                </UpgradeButton>
              </div>
              <Link href="/#pricing" style={{ display: "inline-block", marginTop: "1rem", fontSize: "0.8rem", color: C.pink, textDecoration: "underline" }}>
                See all plans
              </Link>
            </section>
          ) : (
            <section style={{ border: `2px solid ${C.black}`, background: C.white, padding: "1.75rem" }}>
              <label
                ...
                style={{
                  ...
                  border: `2px dashed ${C.black}`, padding: "3rem 1.5rem", textAlign: "center", cursor: "pointer",
                }}
              >
                <span style={{ fontSize: "0.95rem", fontWeight: 700, color: C.black }}>
                  {file ? file.name : "Click to choose a video file"}
                </span>
                <span style={{ marginTop: "0.4rem", fontSize: "0.72rem", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "#888" }}>
                  MP4 or MOV · under ~10 minutes
                </span>
                <input
                  id="video-upload"
                  type="file"
              ...
              <button
                ...
                style={{
                  ...
                  background: !file || state === "uploading" ? "#ccc" : C.cherry,
                  color: C.white, border: `2px solid ${C.black}`, padding: "1rem",
                  fontSize: "0.95rem", fontWeight: 800,
                  cursor: !file || state === "uploading" ? "not-allowed" : "pointer",
                  fontFamily: "inherit",
                }}
              >
                {state === "uploading" ? "Analyzing - this can take a minute" : "Analyze video"}
              </button>

              {state === "error" && errorMessage && (
                <p style={{ marginTop: "1rem", background: "#fee", border: "1px solid #f99", padding: "0.75rem 1rem", fontSize: "0.85rem", color: "#900" }}>
                  {errorMessage}
                </p>
              )}
            </section>
          ...
          {hooks && (
            <section style={{ marginTop: "2.5rem" }}>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: "1.3rem", fontWeight: 800, marginBottom: "1.25rem", color: C.black }}>
                Your ranked hooks
              </h2>
              <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                {hooks.map((h, i) => (
                  <HookCard key={i} hook={h} rank={i + 1} />
                ))}
              </div>
            </section>
          )}
          ...
          {transcript && (
            <details style={{ marginTop: "2rem", border: `1px solid #ddd`, background: "rgba(255,255,255,0.5)", padding: "1rem" }}>
              <summary style={{ cursor: "pointer", fontWeight: 700, fontSize: "0.85rem", color: C.black }}>
                View raw transcript
              </summary>
              <p style={{ marginTop: "0.75rem", whiteSpace: "pre-wrap", fontSize: "0.85rem", color: "#555", lineHeight: 1.6 }}>
                {transcript}
              </p>
            </details>
          )}
  ...
  return (
    <div style={{ border: `2px solid ${C.black}`, background: C.white, padding: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ background: C.cobalt, color: C.white, padding: "0.2rem 0.7rem", fontSize: "0.68rem", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>
          {hook.framework}
        </span>
        <span style={{ fontSize: "0.75rem", color: "#999", fontWeight: 700 }}>#{rank}</span>
      </div>

      <p style={{ marginTop: "1rem", fontFamily: "'Fraunces', serif", fontSize: "1.15rem", fontStyle: "italic", fontWeight: 400, color: C.black, lineHeight: 1.5 }}>
        "{hook.hook}"
      </p>
      ...
      <div style={{ marginTop: "1.25rem", background: C.sand, border: `1px solid #ddd`, padding: "1rem" }}>
        <p style={{ fontSize: "0.68rem", fontWeight: 700, color: "#999", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: "0.4rem" }}>
          Ready-to-post caption
        </p>
        <p style={{ fontSize: "0.85rem", color: C.black, lineHeight: 1.55 }}>{hook.caption}</p>
        <button
          onClick={() => copy(hook.caption, "caption")}
